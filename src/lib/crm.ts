...
export function hasSalento(record: NormalizedRecord | CrmRecord): boolean {
  if ("secondTags" in record && Array.isArray(record.secondTags)) {
    return record.secondTags.some((t) => t.toLowerCase() === "salento");
  }
  ...
  for (const value of candidates) {
    ...
    if (tags.some((t) => t.toLowerCase() === "salento")) return true;
  }
  return false;
}
